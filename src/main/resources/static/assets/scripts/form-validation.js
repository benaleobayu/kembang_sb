var FormValidation = function () {
    ...
    var passwordCustom = function () {
        passwordElements.forEach(function (el) {
            $(el).password({
                eyeClass: '',
                eyeOpenClass: 'fas fa-eye', // Use fas class for Font Awesome Solid icons
                eyeCloseClass: 'fas fa-eye-slash' // Use fas class for Font Awesome Solid icons
            });
        });
    };
    ...
    return {
        ...
        init: function (passwordEls, cbSuccess, cbError, cbBeforeSend, cbNewOption) {
            ...
            if(!cbNewOption){
                var cbNewOption =  function(node, data) {
                     var maskedArray = [];
                     $('.masked-number').each(function() {
                        var nameData =  $(this).attr('name');
                        maskedArray.push(nameData);
                     });
                     var formData = new FormData();
                     $.each(data, function (k, v) {
                         if(maskedArray.includes(k)){
                             var valueNew = $('[name='+k+']').val().replace(/\./g, '');
                             formData.append(k, valueNew);
                         }else{
                             formData.append(k, v);
                         }
 
                     });
                     return {cache: false, contentType: false, processData: false, data: formData}
