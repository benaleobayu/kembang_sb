...
public class BusinessCategoryController {
    ...
    public ResponseEntity<PaginationCmsResponse<ResultPageResponseDTO<BusinessCategoryIndexResponse>>> listDataBusinessCategory(
            ...
            @RequestParam(name = "export", required = false) Boolean export // TODO export LOB
    ) {
        // response true
    ...
    public ResponseEntity<?> getById(@PathVariable("id") String id) {
        ...
        try {
            BusinessCategoryIndexResponse item = service.findDataBySecureId(id);
    ...
    @PostMapping
    public ResponseEntity<?> create(@Valid @RequestBody BusinessCategoryParentCreateRequest item) {
        log.info("POST " + urlRoute + " endpoint hit");
        try {
            service.saveData(item);
            return ResponseEntity.created(URI.create(urlRoute))
                    .body(new ApiAttributeResponse(true, "Successfully created business category"));
        } catch (BadRequestException e) {
            return ResponseEntity.badRequest().body(new ApiResponse(false, e.getMessage()));
        }
    }
